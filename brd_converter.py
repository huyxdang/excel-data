...
def extract_images(sheet):
    """Output cell locations of images from a sheet"""
    images = []
...
def excel_to_csv(excel_path, sheet_name=None):
    """
    Convert Excel sheet to CSV string with cell coordinates
    
    Args:
        excel_path: Path to Excel file
        sheet_name: Name of sheet to convert (None / Default = first sheet)
    
    Returns:
        CSV string with newline separators, including cell coordinates
    """
    # Load workbook
    ...
    for row in sheet.iter_rows(min_row=1, max_row=sheet.max_row):
        ...
        for cell in row:
            ...
            # Get cell value
                ...
                # Empty cell - show coordinate only
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python brd_converter.py <excel_file> [output_csv] [sheet_name]")
        print("\nExample:")
        print("  python brd_converter.py input.xlsx output.csv")
        print("  python brd_converter.py input.xlsx output.csv 'Sheet1'")
        sys.exit(1)
